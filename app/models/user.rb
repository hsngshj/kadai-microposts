class User < ApplicationRecord
    ...
    def favorite(like_micropost)
        self.favorites.find_or_create_by(micropost_id: like_micropost.id)
    end
    
    def unfavorite(like_micropost)
        favorites = self.favorites.find_by(micropost_id: like_micropost.id)
